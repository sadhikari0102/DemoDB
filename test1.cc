...
void q0 (){
	...
	s.CopyRel(relName[0],"s1");
	s.CopyRel(relName[1],"ps1");
	// test write and read
	//s.Write(fileName);
	//s.Print();
	char *cnf = "SELECT s1.s_suppkey FROM supplier AS s1, partsupp AS ps1 WHERE (s1.s_suppkey = ps1.ps_suppkey)";

	yy_scan_string(cnf);
	yyparse();

	//s.Print();
	double result = s.Estimate(boolean, rN, 2);
	cout<<"\n estimate - "<<result<<"\n";
	if(result!=800000)
		cout<<"error in estimating Q1 before apply \n ";
	s.Apply(boolean, rN, 2);
	...
	//char *relName1[] = {"s1","ps1"};

	cnf = "SELECT s1.s_suppkey FROM supplier AS s1 WHERE (s1.s_suppkey > 1000)";
	yy_scan_string(cnf);
	yyparse();

	double dummy = s1.Estimate(boolean, rN, 2);
	cout<<"\n"<<dummy<<"\n";
...
void q4 (){
//slightly modified to pass different sets in every call to apply, rather than passing all the relations everytime
	Statistics s;
