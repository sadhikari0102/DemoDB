...
void OrderMaker::growFromParseTree(NameList* atts, Schema* sch) {
	for(;atts;atts=atts->next, numAtts++) {
		whichAtts[numAtts] = sch->Find(atts->name);
		if(whichAtts[numAtts]==-1) {
			cout<<"\nError growing from parse Tree";
			exit(0);
		}
		whichTypes[numAtts] = sch->FindType(atts->name);

	}
}

void OrderMaker :: Print (ostream& os, int level) {
	level+=2;
	os<<"NumAtts = "<<numAtts;
	for (int i = 0; i < numAtts; i++) {
		os<<"\n"<< (string(3*(level), ' '))<<i<<" : "<<whichAtts[i];
		//printf("%3d: %5d ", i, whichAtts[i]);
		if (whichTypes[i] == Int)
			os<<"Int";
		else if (whichTypes[i] == Double)
...
void CNF :: Print (ostream& os, int level) {
	level+=2;
	for (int i = 0; i < numAnds; i++) {
